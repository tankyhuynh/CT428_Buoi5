...
<script type="text/javascript">
  ...
  function showImage(id){
    ...
    xmlhtpp.onreadystatechange = function(){
      if(xmlhtpp.readyState ==4 && xmlhtpp.status == 200){
        document.getElementById("showImage").innerHTML = xmlhtpp.responseText;
      }else console.log("errrrr");
    } 
    console.log("idsp:" + id);

    xmlhtpp.open("GET", "showImage.php?idsp=" + id, true);
    xmlhtpp.send();
  ...
  function suggestProducts(name){
    var xmlhtpp = new XMLHttpRequest();
    xmlhtpp.onreadystatechange = function(){
      if(xmlhtpp.readyState ==4 && xmlhtpp.status == 200){
        document.getElementById("searchResult").innerHTML = xmlhtpp.responseText;
      }else console.log("errrrr");
    } 
    console.log("Name of product:" + name);

    xmlhtpp.open("GET", "searchProduct.php?tensp=" + name, true);
    xmlhtpp.send();
                ...
                $result=$con->query($sql);
                    ...
                    if($result->num_rows > 0){
                       ...
                       while($row=$result->fetch_assoc()){
                        ...
                        echo "<div align='center'><label>Nhập tên sản phẩm cần tìm kiếm: </label>
                                <input type='text' name='nameOfProduct' onkeyup='suggestProducts(this.value)' />
